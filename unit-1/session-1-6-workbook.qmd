...
# Compare designs by running diagnostics separately
# Note: We compare via printed output rather than combining
# (DeclareDesign's + operator creates duplicate inquiry labels when combining)

cat("=== Simple Randomization (N=200) ===\n")
diag_simple <- diagnose_design(redesign(design_simple, N = 200), sims = 200)
print(diag_simple)

cat("\n=== Blocked Randomization (N=200) ===\n")
diag_blocked <- diagnose_design(redesign(design_blocked, N = 200), sims = 200)
print(diag_blocked)
```

::: {.callout-note}
## Comparing the Results
Look at the **Power** and **SD Estimate** columns. Blocking should reduce variance
(lower SD Estimate) when the blocking variable (education) is predictive of outcomes.
:::
...
# Extract diagnosands data frame and plot power curve
# (diagnosis objects need conversion to data frame for ggplot)
diagnosis_N$diagnosands_df |>
