...
![](images/cate-distribution.png){fig-alt="Histogram showing bimodal distribution of individual treatment effects, with ATE line obscuring two distinct subpopulations" width="80%"}
:::
...
## Pitfalls of Traditional Subgroup Analysis
...
![](images/tree-comparison.png){fig-alt="Side-by-side comparison of a prediction tree (Y-bar in leaves) vs a causal tree (tau-hat in leaves)" width="90%"}

:::: fragment
::: smaller
|   | Prediction Forest | Causal Forest |
|------------------------|------------------------|------------------------|
| **Goal** | Predict $Y$ | Predict $\tau(X)$ |
| **Leaves contain** | $\bar{Y}$ (mean outcome) | $\hat\tau$ (treatment effect) |
| **Splits optimize** | Prediction error | Treatment effect heterogeneity |
| **Key addition** | Bagging + feature subsampling | \+ Honest splitting + local centering |
:::
::::

------------------------------------------------------------------------

## Comparing Split Criteria

::::: columns
::: {.column width="48%"}
### Prediction Tree

**Goal:** Group similar *outcomes*

$$\text{Minimize:} \quad \sum_{i \in L} (Y_i - \bar{Y}_L)^2 + \sum_{i \in R} (Y_i - \bar{Y}_R)^2$$

"Put observations with similar $Y$ values in the same leaf."
:::

::: {.column width="48%"}
### Causal Tree

**Goal:** Group similar *treatment effects*

$$\text{Maximize:} \quad (\hat\tau_L - \hat\tau_R)^2$$

where $\hat\tau_L = \bar{Y}_{L,1} - \bar{Y}_{L,0}$

"Put observations with similar $\tau(x)$ in the same leaf."
:::
:::::

:::: fragment
::: {.callout-warning appearance="simple"}
The causal tree criterion looks at **differences in treatment effects** across child nodes — not differences in outcomes. This is a fundamentally different objective.
:::
::::
...
::: fragment
Split your data into two halves — use one to build the tree, the other to fill in the estimates:

![](images/honest-flow.png){fig-alt="Two-stage honest estimation: splitting sample builds tree structure with question marks in leaves, estimation sample passes through to fill leaves with unbiased treatment effect estimates" width="90%"}
:::
...
**Step 1:** Randomly split the data into a splitting sample ($\mathcal{S}$) and an estimation sample ($\mathcal{E}$). (Typically 50/50, but the ratio is tunable.)
...
::: fragment
![](images/local-centering.png){fig-alt="Side-by-side comparison: raw data showing treated vs control, and residualized data after local centering" width="75%"}
:::

::: {.fragment .smaller}
**Left:** Raw data — baseline outcome variation dominates, hard to see treatment effects. **Right:** After local centering — residualized data reveals the treatment effect signal.
:::
...
-   [View on course website](https://hpm883.ssylvia.io/unit-3/causal-forests-demo.html) (recommended — copy code into your Posit Cloud project)
-   [Open in Google Colab](https://colab.research.google.com/github/unc-hpm-quant/HPM883-Spring2026/blob/main/unit-3/causal-forests-demo.ipynb) (Jupyter notebook with R kernel)
:::
...
::: fragment
![](images/cate-distribution.png){fig-alt="Histogram of estimated CATEs showing bimodal distribution with ATE reference line" width="70%"}
:::

::: fragment
Under the hood, `grf` handles honesty, local centering, and forest-based inference automatically.
:::
...
The first thing to do after fitting a causal forest: **is the heterogeneity real?**
...
::: fragment
![](images/output-calibration.png){fig-alt="Example test_calibration output showing both coefficients significant with three stars" width="75%"}
:::

::: {.fragment .smaller}
-   **mean.forest.prediction** near 1: ATE is well-calibrated
-   **differential.forest.prediction** \> 0 with p \< 0.05: **real heterogeneity detected**
-   If differential is not significant, the "heterogeneity" may be noise
:::
...
::: fragment
![](images/output-varimp.png){fig-alt="Horizontal bar chart showing comorbidities and age as top drivers of treatment effect heterogeneity" width="65%"}
:::

::: {.fragment .smaller}
Variables that appear more often in splits are more important for explaining *treatment effect variation* (not outcome prediction).
:::
...
::: fragment
![](images/output-blp.png){fig-alt="BLP output showing comorbidities and age as significant predictors of treatment effect heterogeneity" width="75%"}
:::

::: {.fragment .smaller}
**Reading this:** Each additional comorbidity increases the treatment *benefit* by 0.76 fewer ER visits (p \< 0.001). Age also matters (p = 0.002). Being female does not significantly predict differential benefit.
:::
...
Rank individuals by predicted CATE, split into quintiles, estimate the ATE within each group.

::: fragment
![](images/gates-example.png){fig-alt="GATES bar chart showing staircase pattern of increasing treatment effects from Q1 to Q5, with ATE reference line" width="75%"}
:::

::: {.fragment .smaller}
**Staircase = real heterogeneity.** Q5 benefits 3x more than Q1. If all bars were equal, heterogeneity is weak or noise.
:::
...
**CLAN** = **C**haracterization of **L**ocal **A**verage treatment effect by **N**eighborhood

Compare covariate means between the most-affected (Q5) and least-affected (Q1) groups.

::: fragment
![](images/output-clan.png){fig-alt="CLAN bar chart showing most-affected group has 1.8 more comorbidities and is 8.3 years older than least-affected" width="70%"}
:::

::: {.fragment .smaller}
The most-benefited patients are older with more comorbidities — the CHW program helps the sickest patients most. **CLAN is descriptive, not causal.** It says *who* benefits, not *why*.
:::
...
The **Rank-Average Treatment Effect (RATE)** evaluates targeting quality. **AUTOC** = **A**rea **U**nder the **T**argeting **O**perator **C**haracteristic.

::: fragment
![](images/output-toc.png){fig-alt="TOC curve showing declining excess benefit as fraction treated increases, with AUTOC shaded area and confidence band" width="70%"}
:::

::: {.fragment .smaller}
**Reading the TOC:** Treat in order of predicted benefit (highest CATE first). Curve above zero = targeting outperforms random assignment. **AUTOC \> 0** (p \< 0.05) = your targeting rule works. Flat at zero = no useful heterogeneity.
:::
